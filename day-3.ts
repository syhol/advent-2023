...
function part1(input: string) {
  ...
  return parts.reduce((total, part) => total + part.number, 0);
}

function parseCells(line: string) {
  return line.split("\n").map((line) => line.split(""));
}
function* traverse(cells: string[][]): Generator<Point> {
  for (const row of cells.keys()) {
    const rowValues = cells[row];
    for (const column of rowValues.keys()) {
      const value = rowValues[column];
      yield { row, column, value };
    }
  }
}
function isDigit({ value }: Point): boolean {
  return /[\d]/.test(value);
}
function* groupParts(digits: Point[]): Generator<Part> {
  let last = digits.shift() as Point;
  let part = { points: [last], number: Number(last.value) };
  for (const point of digits) {
    if (point.column === last.column + 1 && point.row === last.row) {
      part.points.push(point);
      part.number = Number(String(part.number) + point.value);
    } else {
      yield part;
      part = { points: [point], number: Number(point.value) };
    }
    last = point;
  }
  yield part;
}
function isPartNumber(cells: string[][]): (number: Part) => boolean {
  return (part: Part) => {
    for (const point of part.points) {
      const candidates = [
        cells.at(point.row - 1)?.at(point.column - 1),
        cells.at(point.row - 1)?.at(point.column),
        cells.at(point.row - 1)?.at(point.column + 1),
        cells.at(point.row)?.at(point.column - 1),
        cells.at(point.row)?.at(point.column + 1),
        cells.at(point.row + 1)?.at(point.column - 1),
        cells.at(point.row + 1)?.at(point.column),
        cells.at(point.row + 1)?.at(point.column + 1),
      ];
      if (candidates.some(isSymbol)) {
        return true;
      }
    }
    return false;
...
function isSymbol(value: string | undefined): boolean {
  return /[^0-9\.\n]/.test(value ?? "");
}
